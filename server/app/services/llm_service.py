"""
LLM Service — Central wrapper for AI models.
Supports: Groq (primary) → Google Gemini → Zhipu GLM-4 (fallbacks).
All AI features route through this service for consistent
error handling, retries, and token tracking.
"""
import os
import re
import json
import asyncio
import logging
from typing import Optional, Dict, Any, List
from dotenv import load_dotenv

load_dotenv()

logger = logging.getLogger(__name__)

# ─── API Configuration ────────────────────────────────────────
GROQ_API_KEY = os.getenv("Groq_API_key", "") or os.getenv("GROQ_API_KEY", "")
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
ZHIPU_API_KEY = os.getenv("ZHIPU_API_KEY", "")
GLM_MODEL = os.getenv("GLM_MODEL", "glm-4-plus")

# Pick provider: Groq > Gemini > Zhipu
if GROQ_API_KEY:
    LLM_PROVIDER = "groq"
elif GEMINI_API_KEY:
    LLM_PROVIDER = "gemini"
elif ZHIPU_API_KEY:
    LLM_PROVIDER = "zhipu"
else:
    LLM_PROVIDER = "none"

logger.info(f"LLM Provider: {LLM_PROVIDER.upper()}")

# ─── Hardening Constants ──────────────────────────────────────
LLM_TIMEOUT_SECONDS = 30
LLM_MAX_RETRIES = 2
LLM_MAX_TOKENS_CAP = 4096
LLM_MAX_PROMPT_CHARS = 8000

_CONTROL_CHAR_RE = re.compile(r'[\x00-\x08\x0b-\x0c\x0e-\x1f\x7f]')


def _sanitize_input(text: str, max_chars: int = LLM_MAX_PROMPT_CHARS) -> str:
    """Strip control chars and truncate to max_chars."""
    cleaned = _CONTROL_CHAR_RE.sub('', text)
    if len(cleaned) > max_chars:
        logger.warning(f"Input truncated from {len(cleaned)} to {max_chars} chars")
        cleaned = cleaned[:max_chars] + "\n[...truncated]"
    return cleaned


def _parse_json_content(content: str) -> Any:
    """Robustly extract JSON from LLM response text."""
    content_stripped = content.strip()
    content_stripped = re.sub(r'^```(?:json)?\s*', '', content_stripped)
    content_stripped = re.sub(r'\s*```$', '', content_stripped)
    content_stripped = content_stripped.strip()
    if not content_stripped.startswith(('{', '[')):
        m = re.search(r'[{\[]', content_stripped)
        if m:
            content_stripped = content_stripped[m.start():]
    return json.loads(content_stripped)


# ─── Groq Client (primary) ───────────────────────────────────

async def _query_groq(
    prompt: str,
    system_prompt: str,
    json_mode: bool = False,
    temperature: float = 0.7,
    max_tokens: int = 2048,
) -> Dict[str, Any]:
    """Query Groq API using llama models (super fast)."""
    try:
        from groq import Groq
    except ImportError:
        return {"content": None, "error": "groq package not installed. Run: pip install groq", "success": False}

    client = Groq(api_key=GROQ_API_KEY)

    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": prompt},
    ]

    kwargs: Dict[str, Any] = {
        "model": "llama-3.3-70b-versatile",
        "messages": messages,
        "temperature": temperature,
        "max_tokens": min(max_tokens, LLM_MAX_TOKENS_CAP),
    }

    if json_mode:
        kwargs["response_format"] = {"type": "json_object"}

    last_error: Exception = Exception("Unknown error")
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            loop = asyncio.get_event_loop()
            response = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: client.chat.completions.create(**kwargs)),
                timeout=LLM_TIMEOUT_SECONDS,
            )

            content = response.choices[0].message.content
            usage = {
                "prompt_tokens": response.usage.prompt_tokens,
                "completion_tokens": response.usage.completion_tokens,
                "total_tokens": response.usage.total_tokens,
            }

            logger.info(f"Groq call success — model=llama-3.3-70b, tokens={usage['total_tokens']}, attempt={attempt + 1}")

            if json_mode:
                try:
                    parsed = _parse_json_content(content)
                    return {"content": parsed, "raw": content, "usage": usage, "success": True}
                except json.JSONDecodeError as je:
                    logger.warning(f"Groq returned non-JSON despite json_mode=True: {je}")
                    return {"content": content, "raw": content, "usage": usage, "success": True, "json_error": True}

            return {"content": content, "usage": usage, "success": True}

        except asyncio.TimeoutError:
            last_error = TimeoutError(f"Groq call timed out after {LLM_TIMEOUT_SECONDS}s")
            logger.error(f"Groq timeout on attempt {attempt + 1}")
            break
        except Exception as e:
            last_error = e
            err_str = str(e).lower()
            is_transient = any(kw in err_str for kw in ["connection", "timeout", "503", "502", "500", "network", "rate"])
            if is_transient and attempt < LLM_MAX_RETRIES:
                wait = 2 ** attempt
                logger.warning(f"Groq transient error (attempt {attempt + 1}), retrying in {wait}s: {e}")
                await asyncio.sleep(wait)
            else:
                logger.error(f"Groq call failed (attempt {attempt + 1}): {e}")
                break

    return {"content": None, "error": str(last_error), "success": False}


# ─── Gemini Client ────────────────────────────────────────────

async def _query_gemini(
    prompt: str,
    system_prompt: str,
    json_mode: bool = False,
    temperature: float = 0.7,
    max_tokens: int = 2048,
) -> Dict[str, Any]:
    """Query Google Gemini API."""
    try:
        import google.generativeai as genai
    except ImportError:
        return {"content": None, "error": "google-generativeai not installed", "success": False}

    genai.configure(api_key=GEMINI_API_KEY)
    model = genai.GenerativeModel(
        model_name="gemini-2.0-flash",
        system_instruction=system_prompt,
        generation_config=genai.GenerationConfig(
            temperature=temperature,
            max_output_tokens=min(max_tokens, LLM_MAX_TOKENS_CAP),
            response_mime_type="application/json" if json_mode else "text/plain",
        ),
    )

    last_error: Exception = Exception("Unknown error")
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            loop = asyncio.get_event_loop()
            response = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: model.generate_content(prompt)),
                timeout=LLM_TIMEOUT_SECONDS,
            )
            content = response.text
            usage = {}
            try:
                um = response.usage_metadata
                if um:
                    usage = {
                        "prompt_tokens": getattr(um, 'prompt_token_count', 0),
                        "completion_tokens": getattr(um, 'candidates_token_count', 0),
                        "total_tokens": getattr(um, 'total_token_count', 0),
                    }
            except Exception:
                pass

            logger.info(f"Gemini call success — tokens={usage.get('total_tokens', '?')}, attempt={attempt + 1}")

            if json_mode:
                try:
                    parsed = _parse_json_content(content)
                    return {"content": parsed, "raw": content, "usage": usage, "success": True}
                except json.JSONDecodeError as je:
                    logger.warning(f"Gemini returned non-JSON: {je}")
                    return {"content": content, "raw": content, "usage": usage, "success": True, "json_error": True}

            return {"content": content, "usage": usage, "success": True}

        except asyncio.TimeoutError:
            last_error = TimeoutError(f"Gemini timed out after {LLM_TIMEOUT_SECONDS}s")
            break
        except Exception as e:
            last_error = e
            if attempt < LLM_MAX_RETRIES:
                await asyncio.sleep(2 ** attempt)
            else:
                break

    return {"content": None, "error": str(last_error), "success": False}


# ─── Zhipu Client ────────────────────────────────────────────

async def _query_zhipu(
    prompt: str,
    system_prompt: str,
    json_mode: bool = False,
    temperature: float = 0.7,
    max_tokens: int = 2048,
    model: Optional[str] = None,
) -> Dict[str, Any]:
    """Query Zhipu GLM-4 API."""
    try:
        from zhipuai import ZhipuAI
    except ImportError:
        return {"content": None, "error": "zhipuai not installed", "success": False}

    client = ZhipuAI(api_key=ZHIPU_API_KEY)
    use_model = model or GLM_MODEL

    kwargs: Dict[str, Any] = {
        "model": use_model,
        "messages": [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": prompt},
        ],
        "temperature": temperature,
        "max_tokens": min(max_tokens, LLM_MAX_TOKENS_CAP),
    }

    last_error: Exception = Exception("Unknown error")
    for attempt in range(LLM_MAX_RETRIES + 1):
        try:
            loop = asyncio.get_event_loop()
            response = await asyncio.wait_for(
                loop.run_in_executor(None, lambda: client.chat.completions.create(**kwargs)),
                timeout=LLM_TIMEOUT_SECONDS,
            )
            content = response.choices[0].message.content
            usage = {
                "prompt_tokens": response.usage.prompt_tokens,
                "completion_tokens": response.usage.completion_tokens,
                "total_tokens": response.usage.total_tokens,
            }

            if json_mode:
                try:
                    parsed = _parse_json_content(content)
                    return {"content": parsed, "raw": content, "usage": usage, "success": True}
                except json.JSONDecodeError:
                    return {"content": content, "raw": content, "usage": usage, "success": True, "json_error": True}

            return {"content": content, "usage": usage, "success": True}

        except asyncio.TimeoutError:
            break
        except Exception as e:
            last_error = e
            if attempt < LLM_MAX_RETRIES:
                await asyncio.sleep(2 ** attempt)
            else:
                break

    return {"content": None, "error": str(last_error), "success": False}


# ─── Unified query_llm ────────────────────────────────────────

async def query_llm(
    prompt: str,
    system_prompt: str = "You are a helpful procurement assistant.",
    json_mode: bool = False,
    temperature: float = 0.7,
    max_tokens: int = 2048,
    model: Optional[str] = None,
) -> Dict[str, Any]:
    """
    Unified LLM query — routes to Groq → Gemini → Zhipu based on config.
    """
    if LLM_PROVIDER == "none":
        return {"content": None, "error": "No LLM API key configured. Set Groq_API_key, GEMINI_API_KEY, or ZHIPU_API_KEY in .env", "success": False}

    prompt = _sanitize_input(prompt, LLM_MAX_PROMPT_CHARS)
    system_prompt = _sanitize_input(system_prompt, 2000)

    if LLM_PROVIDER == "groq":
        return await _query_groq(prompt, system_prompt, json_mode, temperature, max_tokens)
    elif LLM_PROVIDER == "gemini":
        return await _query_gemini(prompt, system_prompt, json_mode, temperature, max_tokens)
    else:
        return await _query_zhipu(prompt, system_prompt, json_mode, temperature, max_tokens, model)


async def query_llm_with_context(
    question: str,
    context_data: Dict[str, Any],
    system_prompt: Optional[str] = None,
) -> Dict[str, Any]:
    """RAG-style query: send question + database context to LLM."""
    default_system = """You are an AI procurement assistant with access to live company data.
Answer questions accurately using ONLY the data provided in the context.
Be specific with numbers, dates, and names. Keep answers concise but helpful.
If the data doesn't contain enough information to answer, say so clearly.
Format currency amounts with $ and commas. Use bullet points for lists."""

    prompt = f"""CONTEXT (live company data):
{json.dumps(context_data, indent=2, default=str)}

USER QUESTION: {question}

Answer based on the context above. Be specific and cite actual numbers."""

    return await query_llm(
        prompt=prompt,
        system_prompt=system_prompt or default_system,
        temperature=0.3,
        max_tokens=1024,
    )


# ─── Feature-Specific Functions ──────────────────────────────

async def parse_natural_language_request(
    user_input: str,
    product_catalog: List[Dict],
    current_stock: Optional[List[Dict]] = None,
) -> Dict[str, Any]:
    """Parse a natural language purchase request into structured PR data."""
    system_prompt = """You are a procurement assistant that converts natural language purchase requests
into structured data. You must:
1. Extract product items, quantities, and specifications
2. Match items to the product catalog when possible
3. Suggest urgency level based on language cues
4. Identify the department if mentioned

Return ONLY valid JSON in this exact format:
{
  "title": "Short title for this purchase request",
  "department": "department name or null",
  "urgency": "low|medium|high|critical",
  "purpose": "brief purpose description",
  "items": [
    {
      "item_name": "matched product name",
      "matched_sku": "SKU if matched, null otherwise",
      "matched_product_id": "product ID if matched, null otherwise",
      "quantity": number,
      "unit": "pcs|boxes|reams|packs|kg|liters",
      "estimated_unit_price": number or 0,
      "reason": "why this item and quantity"
    }
  ],
  "needed_by_days": number of days from now or null,
  "ai_notes": "any additional observations"
}"""

    catalog_summary = [
        {"id": str(p.get("id", "")), "name": p.get("name", ""), "sku": p.get("sku", ""),
         "category": p.get("category", ""), "unit": p.get("unit", "pcs"), "price": p.get("price", 0)}
        for p in product_catalog[:50]
    ]

    prompt = f"""PRODUCT CATALOG:
{json.dumps(catalog_summary, indent=2)}

{"CURRENT STOCK LEVELS:" + json.dumps(current_stock[:30], indent=2) if current_stock else ""}

USER REQUEST: "{user_input}"

Parse this into a structured purchase requisition. Match to catalog products where possible."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, json_mode=True, temperature=0.2)


async def generate_supplier_explanation(
    product_name: str, quantity: int, supplier_scores: List[Dict], urgency: str = "medium",
) -> Dict[str, Any]:
    """Generate human-friendly supplier recommendation explanation."""
    system_prompt = """You are a procurement advisor. Given supplier scoring data,
write a clear, actionable recommendation in 3-5 sentences.
Explain WHY the top supplier is best. Use ✅ for strengths and ⚠️ for concerns."""

    prompt = f"""PURCHASE: {quantity}x {product_name} (Urgency: {urgency})

SUPPLIER SCORES (ranked by AI):
{json.dumps(supplier_scores, indent=2)}

Write a procurement recommendation explaining which supplier to choose and why."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, temperature=0.5)


async def parse_supplier_email(email_text: str, product_catalog: List[Dict]) -> Dict[str, Any]:
    """Parse a supplier email/quote into structured data."""
    system_prompt = """You are a procurement data extraction system.
Extract quote information from supplier emails. Match products to the catalog when possible.
Return ONLY valid JSON:
{
  "supplier_name": "name",
  "responding_to": "RFQ number or null",
  "valid_until": "YYYY-MM-DD or null",
  "delivery_days": "estimated days or range",
  "payment_terms": "terms string",
  "items": [
    {"description": "item", "catalog_match_name": "match or null", "catalog_match_id": "id or null",
     "quoted_price": number, "available": true, "moq": null, "substitute_offered": null}
  ],
  "notes": "any other relevant info"
}"""

    catalog_summary = [{"id": str(p.get("id", "")), "name": p.get("name", ""), "sku": p.get("sku", "")}
                       for p in product_catalog[:50]]

    prompt = f"""PRODUCT CATALOG: {json.dumps(catalog_summary)}

SUPPLIER EMAIL:
{email_text}

Extract all quote data. Match products to catalog."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, json_mode=True, temperature=0.1)


async def parse_price_sheet_text(raw_text: str, product_catalog: List[Dict]) -> Dict[str, Any]:
    """Parse OCR-extracted text from a price sheet into structured data."""
    system_prompt = """You are a document parsing system for procurement.
Extract product names, prices, and units from price sheet text.
Return ONLY valid JSON:
{
  "supplier_prices": [
    {"description": "item", "matched_product_name": "match or null", "matched_product_id": "id or null",
     "price": number, "unit": "unit", "confidence": 0.0 to 1.0}
  ],
  "valid_until": "date or null", "payment_terms": "terms or null",
  "minimum_order": null, "currency": "USD"
}"""

    catalog_summary = [{"id": str(p.get("id", "")), "name": p.get("name", ""), "sku": p.get("sku", "")}
                       for p in product_catalog[:50]]

    prompt = f"""PRODUCT CATALOG: {json.dumps(catalog_summary)}

PRICE SHEET TEXT (OCR):
{raw_text}

Extract all prices and match to catalog products."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, json_mode=True, temperature=0.1)


async def generate_po_draft(
    po_data: Dict[str, Any], supplier: Dict[str, Any], line_items: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """Generate a professional PO email draft and document."""
    system_prompt = """You are a professional procurement officer writing purchase orders.
Generate a formal, polite PO email and a structured PO document.
Return ONLY valid JSON:
{
  "email_subject": "Purchase Order PO-XXXX - [brief description]",
  "email_body": "Full professional email text to send to supplier",
  "po_document": "Formatted plain-text PO document with all line items",
  "special_notes": ["any important notes or flags"],
  "estimated_delivery_note": "suggested delivery timeline note"
}"""

    items_text = "\n".join([
        f"- {item.get('product_name', 'Item')} | Qty: {item.get('quantity', 0)} {item.get('unit', 'pcs')} | "
        f"Unit Price: ${item.get('unit_price', 0):.2f} | Total: ${item.get('total_price', 0):.2f}"
        for item in line_items
    ])

    prompt = f"""Generate a professional Purchase Order:

PO NUMBER: {po_data.get('po_number', 'PO-DRAFT')}
TOTAL AMOUNT: ${po_data.get('total_amount', 0):.2f}
PAYMENT TERMS: {po_data.get('payment_terms', 'Net 30')}
PURPOSE: {po_data.get('purpose', 'General procurement')}

SUPPLIER: {supplier.get('name', 'Supplier')} ({supplier.get('email', 'N/A')})

LINE ITEMS:
{items_text}

Generate a professional PO email and document."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, json_mode=True, temperature=0.3)


async def match_invoice_to_po(
    invoice_text: str, po_data: Dict[str, Any], received_items: Optional[List[Dict]] = None,
) -> Dict[str, Any]:
    """Perform 3-way invoice matching: PO vs Invoice vs Goods Received."""
    system_prompt = """You are an accounts payable AI performing 3-way invoice matching.
Compare the invoice text against the PO data and goods received.
Return ONLY valid JSON:
{
  "invoice_extracted": {
    "invoice_number": "string or null", "invoice_date": "date or null",
    "supplier_name": "string or null", "total_amount": number or null,
    "line_items": [{"description": "item", "quantity": number, "unit_price": number, "total": number}],
    "payment_terms": "string or null", "due_date": "string or null"
  },
  "match_result": "approved" or "discrepancy" or "rejected",
  "match_score": 0.0 to 1.0,
  "discrepancies": [
    {"type": "price_mismatch", "description": "explanation", "po_value": "x", "invoice_value": "y", "financial_impact": number}
  ],
  "recommended_action": "approve_for_payment" or "request_credit_note" or "contact_supplier" or "reject",
  "action_reason": "explanation",
  "total_dispute_amount": number
}"""

    prompt = f"""INVOICE TEXT:
{invoice_text}

PURCHASE ORDER:
PO Number: {po_data.get('po_number', 'N/A')}
PO Total: ${po_data.get('total_amount', 0):.2f}
Supplier: {po_data.get('supplier_name', 'N/A')}
PO Line Items: {json.dumps(po_data.get('line_items', []), indent=2)}

GOODS RECEIVED: {json.dumps(received_items or [], indent=2)}

Perform 3-way matching and identify all discrepancies."""

    return await query_llm(prompt=prompt, system_prompt=system_prompt, json_mode=True, temperature=0.1)
